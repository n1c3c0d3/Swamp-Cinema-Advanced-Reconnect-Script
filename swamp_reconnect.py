    ...
    while True:
        try:
            ...
            if gmod_running and udp_active:
                ...
                launch_gmod()
                if crash_attempts >= 3:
                    log_message("⚠️ [INFO] 3 consecutive failed launches detected. Running GModPatchTool...")
                    if run_gmod_patch_tool():
                        crash_attempts = 0
                    else:
                        log_message("⚠️ [INFO] Patch tool failed after repeated launch issues. Validating game files...")
